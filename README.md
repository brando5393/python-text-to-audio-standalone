...
## Features
- **Supported formats**: `.txt`, `.md`, `.pdf`, `.epub`, non-DRM `.mobi`/`.azw3`/`.azw`/`.prc`, `.docx`, `.rtf`, `.html`/`.htm`/`.xhtml`, `.fb2` (FictionBook), `.pptx`, and `.odt` (OpenDocument Text). DRM-locked Kindle store books can't be decrypted by this app, or legally by anyone without the device's Kindle key, so that's a hard limit rather than a bug.
...
- **Progress dialog**: converting shows a per-file and overall progress bar with an estimated time remaining, plus a Cancel button, so there's no wondering whether a long book is still working or stuck. Closing or hiding this window never stops the conversion itself; a **Show Conversion Progress** button stays available in the Actions panel while a batch is running, so you can bring the window back at any time and it catches up to the current state immediately.
...
3. Install dependencies with [Poetry](https://python-poetry.org/):
   ...
   ```
   Or with plain pip: `pip install pypdf pyttsx3 ttkbootstrap ebooklib beautifulsoup4 mobi python-docx striprtf packaging sounddevice numpy soundfile python-pptx odfpy` then `python main.py`.

## Usage
1. Launch the app. **Add Files** to queue `.txt`/`.pdf`/`.epub`/`.mobi`/`.azw3` (and other supported formats -- see above) files for conversion.
...
| `TextExtraction.py` | Pulls plain text out of txt/md/pdf/epub/mobi/azw3/azw/prc/docx/rtf/html/xhtml/fb2/pptx/odt, reads page/chapter/slide counts for display (falling back to a text-based heuristic estimate when a format has no real structural metadata), and (for EPUB/DOCX/MOBI/AZW3/AZW/PRC/FB2) splits a document into per-chapter text for the "Split into chapter files" option |
